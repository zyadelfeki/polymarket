#!/usr/bin/env python3
"""
approve_funds.py — Approve USDC allowance for the Polymarket CLOB contract.

Exit codes (see infra/errors.py ExitCode):
  0   — OK
  13  — BAD_CONFIG  (missing env vars)
  21  — RPC_ERROR
  22  — BAD_PRIVATE_KEY
  20  — APPROVAL_TX_FAILED
  99  — UNKNOWN_ERROR
"""
import sys
import os
...
def _load_config() -> dict:
    """Load required env vars.  Raises ValueError if any are missing."""
...
def _approve(cfg: dict) -> str:
    """
    Submit the USDC approval transaction.
    ...
    SPENDER        = "0x4bFb41d5B3570DeFd03C39a9A4D8dE6Bd8B8982E"  # CLOB contract
    MAX_UINT256    = 2**256 - 1
    ERC20_APPROVE_ABI = [
        {
            "name": "approve",
    ...
    if receipt.status != 1:
        raise RuntimeError(
            f"Approval tx reverted. Hash: {tx_hash.hex()} Status: {receipt.status}"
        )
...
def main() -> int:
    logger.info("approve_funds_start")

    try:
        cfg = _load_config()
    except ValueError as exc:
        logger.error(
            "approve_funds_failed",
    ...
    logger.info("approve_funds_ok", tx_hash=tx_hash)
    return ExitCode.OK
