...
async def test_execute_trade_returns_true_on_success():
    ...
    Design note: circuit_breaker.record_trade() is intentionally NOT called
    here.  A placement is an open position, not a resolved outcome.
    The settlement loop in run_paper_trading.py calls it after the market
    window has closed.  Tests must not assert that record_trade is called
    during placement — that assertion would contradict the invariant.
    """
    from execution.trade_executor import TradeExecutor
...
async def test_execute_trade_min_bet_clamp():
    """
    When kelly_size < MIN_BET_SIZE but balance >= MIN_BET_SIZE, the executor
    ...
    assert placed_sizes[0] == MIN_BET_SIZE, (
        f"Expected executor to clamp bet to {MIN_BET_SIZE}, got {placed_sizes[0]}"
    )
...
async def test_execute_trade_rejected_below_minimum_insufficient_balance():
    """
    When kelly_size < MIN_BET_SIZE AND balance < MIN_BET_SIZE, the executor
    ...
    mock_bankroll = MagicMock()
    mock_bankroll.current_balance = Decimal("0.50")  # below floor
    mock_db = MagicMock()
    mock_breaker = MagicMock()
    mock_breaker.is_trading_allowed.return_value = True
