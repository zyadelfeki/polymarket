...
class CircuitBreakerV2:
    ...
    async def trip(
        ...
        """
        Public async trip entry-point called from main.py monitoring loops.

        Accepts a ``TripReason`` enum value (preferred) or falls back to
        ``TripReason.MANUAL``.  Unlike ``manual_trip`` (which always uses
        ``TripReason.MANUAL``), this method preserves the caller-supplied reason
        so drawdown and loss-streak trips are logged with the correct label.

        Args:
            reason:       TripReason enum member.  Defaults to TripReason.MANUAL.
            drawdown_pct: Current drawdown fraction (0–1) for context logging.
            equity:       Current equity snapshot; uses ``self.peak_equity`` if None.
        """
        if reason is None:
            reason = TripReason.MANUAL
        if not isinstance(reason, TripReason):
            # Defensive coercion: accept string values from legacy callers.
            try:
                reason = TripReason(str(reason))
            except ValueError:
                reason = TripReason.MANUAL

        effective_equity = equity if equity is not None else self.peak_equity

        async with self.state_lock:
            if self.state != CircuitState.OPEN:
                await self._trip_circuit(reason, effective_equity, drawdown_pct)
                logger.warning(
                    "circuit_breaker_programmatic_trip",
                    reason=reason.value,
                    drawdown_pct=drawdown_pct,
                )
