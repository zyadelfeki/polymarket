...
class BankrollTracker:
    ...
    def get_total_return(self) -> Decimal:
        """Total return percentage (Decimal)."""
        if self.initial_capital > 0:
            return (self.current_capital - self.initial_capital) / self.initial_capital * Decimal("100")
        return Decimal("0")
    
    def get_win_rate(self) -> Decimal:
        """Calculate win rate percentage (Decimal)."""
        if self.total_trades > 0:
            return (Decimal(self.winning_trades) / Decimal(self.total_trades)) * Decimal("100")
        return Decimal("0")
    
    def get_sharpe_ratio(self) -> Decimal:
        """Calculate Sharpe ratio (simplified, Decimal)."""
        if len(self.trade_history) < 2:
            return Decimal("0")

        returns = [Decimal(str(t["profit"])) for t in self.trade_history]
    ...
    def print_summary(self):
        ...
        print("="*60)
        print(
            f"💵 Capital: ${stats['current_capital'].quantize(Decimal('0.01'))} "
            f"(from ${stats['initial_capital'].quantize(Decimal('0.01'))})"
        )
        print(f"📈 Total Return: {stats['total_return_pct'].quantize(Decimal('0.1')):+}%")
        print(
            f"🎯 Win Rate: {stats['win_rate_pct'].quantize(Decimal('0.1'))}% "
            f"({stats['winning_trades']}/{stats['total_trades']})"
        )
        print(f"📊 Sharpe Ratio: {stats['sharpe_ratio'].quantize(Decimal('0.01'))}")
        print(f"🔺 Peak: ${stats['peak_capital'].quantize(Decimal('0.01'))}")
        print("="*60 + "\n")