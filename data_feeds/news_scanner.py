...
Monitors breaking crypto/finance news from NewsAPI and CryptoPanic
"""
import aiohttp
import asyncio
...
class NewsItem:
    ...
    def __init__(self, title: str, description: str, source: str, 
                 published_at: datetime, url: str, relevance_score: float = 0.0):
        self.title = title
...
class NewsScanner:
    """Real-time news monitoring across multiple sources"""
    ...
    async def fetch_newsapi(self, session: aiohttp.ClientSession, 
                           lookback_minutes: int = 10) -> List[NewsItem]:
                    ...
                    for post in posts[:15]:
                        try:
                            news_item = NewsItem(
                                ...
                                url=post.get("url", ""),
                                relevance_score=self._score_relevance({"title": post.get("title", "")})
                            )
                            news_items.append(news_item)
                        except:
                            continue
    ...
    def _score_relevance(self, article: Dict) -> float:
        """Score article relevance 0.0-1.0"""
        text = f"{article.get('title', '')} {article.get('description', '')}".lower()
        score = sum(0.15 for keyword in self.priority_keywords if keyword in text)
