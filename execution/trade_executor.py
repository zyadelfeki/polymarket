...
class TradeExecutor:
    ...
    async def process_execution_queue(self):
        ...
        cannot kill the consumer coroutine.  The error is logged with full
        context so the root cause is always diagnosable.  task_done() is
        called in finally so Queue.join() unblocks correctly even on failure.
        """
        while True:
