...
- All structured logging uses structlog where available, stdlib otherwise.
...
class TradeExecutor:
    ...
    async def execute_trade(self, opportunity: Dict) -> bool:
        ...
        # manual / test callers that don't go through the scanner).
        if "kelly_size" in opportunity and opportunity["kelly_size"] is not None:
            bet_size = Decimal(str(opportunity["kelly_size"]))
        else:
            # Legacy path: re-derive size from kelly_sizer.  This path is only
            # reached when the opportunity was not generated by BTCPriceLevelScanner.
            # For gate-generated opportunities this should never execute.
